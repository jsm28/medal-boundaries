...
\usepackage[a4paper,left=2cm,right=2cm,bottom=2.5cm,top=2.5cm]{geometry}
\usepackage{hyperref}
\begin{document}
...
practice; the following section deals with such algorithms adapted to
determine all three boundaries together in a similar way.
...
\section*{Unified determination of medal boundaries}

In this section, the algorithms from the previous section are adapted
to determine the bronze medal boundary together with the others, on
the basis of approximating a ratio of $1:2:3:6$ when `no medal' is
considered as a kind of medal, and included in all the summations
described above as such.  The following table shows when each of these
unified approaches has failed to predict the boundaries chosen by the
Jury.  The boundaries actually chosen are in bold; other choices of
boundaries have the algorithms that would have chosen them indicated.
Stricter choices of boundaries (fewer medals, then fewer gold and
silver medals, then fewer gold medals) are on the left.  The code was
originally written to determine just gold and silver boundaries and
has not been checked carefully for correctness for determining all
boundaries at once.

\input{gen-gsb-alg-table.tex}
