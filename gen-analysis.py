...
def gen_gsb_alg_table(data):
    """
    Generate list of years where unified algorithms fail to predict
    ...
    algs = (('I', MedalAlgorithmIndependent()),
            ...
            ('L^2s', MedalAlgorithmLp(2, True)),
            ('R', MedalAlgorithmRatio()))
    with open('gen-gsb-alg-table.tex', 'w') as tex_file:
        ydata = {}
        for year in range(START_YEAR, NEXT_YEAR):
            d = data[year]
            dt = (d.cum_medal_stats[2], d.cum_medal_stats[1],
        ...
        for year in range(START_YEAR, NEXT_YEAR):
            ...
            for k in sorted(yd.keys()):
                if 'Actual' in yd[k]:
                    ylist.append('{\\tiny\\textbf{(%d, %d, %d)}}'
                                 % (k[2], k[1]-k[2], k[0]-k[1]))
                else:
                    ylist.append('{\\tiny(%d, %d, %d)} {\\tiny $%s$}'
                                 % (k[2], k[1]-k[2], k[0]-k[1],
                                    ''.join(yd[k])))
            ylist.extend(['' for i in range(max_num_opts - len(ylist))])
            tex_file.write('%d & %s\\\\\n' % (year, ' & '.join(ylist)))
